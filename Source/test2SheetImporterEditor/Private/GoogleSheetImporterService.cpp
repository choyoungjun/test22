...
static bool FetchCsvSync(const FString& Url, FString& OutCsv, FString& OutError)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[&bDone, &bSuccess, &OutCsv, &OutError](FHttpRequestPtr /*RequestPtr*/, FHttpResponsePtr Response, bool bConnectedSuccessfully)
		{
			bDone = true;
			...
			OutCsv = Response->GetContentAsString();
			bSuccess = true;
		});
	...
	FGoogleSheetImportResult& OutResult)
{
	...
	const int32 NameIdx = Header.IndexOfByPredicate([](const FString& V) { return V.Equals(TEXT("Name"), ESearchCase::IgnoreCase); });
	const int32 ValueIdx = Header.IndexOfByPredicate([](const FString& V) { return V.Equals(TEXT("Value"), ESearchCase::IgnoreCase); });
	const int32 DisplayIdx = Header.IndexOfByPredicate([](const FString& V) { return V.Equals(TEXT("DisplayName"), ESearchCase::IgnoreCase); });
	const int32 CommentIdx = Header.IndexOfByPredicate([](const FString& V) { return V.Equals(TEXT("Comment"), ESearchCase::IgnoreCase); });
	...
	FGoogleSheetImportResult& OutResult)
{
	...
	if (OutSheet.RowNameColumnIndex == INDEX_NONE)
	{
		OutSheet.RowNameColumnIndex = 0;
	}
...
static FString BuildCsvForDataTable(const FParsedTableSheet& TableSheet)
{
	...
	for (const TArray<FString>& Row : TableSheet.Rows)
	{
		const FString RowNameValue = Row.IsValidIndex(TableSheet.RowNameColumnIndex) ? ToSafeIdentifier(Row[TableSheet.RowNameColumnIndex]) : FString();
...
FGoogleSheetImportResult FGoogleSheetImporterService::Execute(const EGoogleSheetImportAction Action)
{
	...
	Workbook.Enums.Sort([](const FParsedEnumSheet& L, const FParsedEnumSheet& R) { return L.EnumName < R.EnumName; });
	Workbook.Tables.Sort([](const FParsedTableSheet& L, const FParsedTableSheet& R) { return L.StructName < R.StructName; });
